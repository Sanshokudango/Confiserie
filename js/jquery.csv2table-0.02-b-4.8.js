...
(function($) {
	...
	$.fn.csv2table= function (url,setting){
		...
		function chkCompOpr(siki){
			return siki.match(/^&&|\|\|$/g) && siki.length==8 //lengthが配列？の数の上限rowAry[i]['ここ']
		}
			...
			for (var i=s; i<row; i++) {
					...
					//for (var j=0; j<col; j++) {
						...
						htm+= '<td>'
						   ...
						   +"&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;"+rowsAry[i][5]+'</p>'
						   +"&emsp;&emsp;"+'<a href="'+rowsAry[i][10]+'">'+"HP"+'</a>'+"&emsp;/&emsp;"
						   +'<a href="'+rowsAry[i][9]+'">'+"TEL"+'</a>'+"&emsp;/&emsp;"
						   +'<a href="'+rowsAry[i][12]+","+rowsAry[i][13]+'">'+"NAVI"+'</a>'
						   +'<br clear="left">'
						   +'<br>'
